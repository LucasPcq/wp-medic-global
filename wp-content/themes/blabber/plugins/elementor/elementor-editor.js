...
jQuery( document ).ready(
	function() {
		...
		blabber_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="blabber_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.blabber_post_editor' );
					if ( link.length > 0 ) {
						blabber_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.blabber_post_editor', function(e) {
					blabber_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('blabber_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function blabber_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
