...
if ( ! function_exists( 'blabber_need_page_title' ) ) {
	function blabber_need_page_title() {
		return ! is_front_page() && apply_filters( 'blabber_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'blabber_show_layout' ) ) {
	function blabber_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'blabber_get_logo_image' ) ) {
	function blabber_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = blabber_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = blabber_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = blabber_is_on( blabber_get_theme_option( 'logo_retina_enabled' ) )
						? blabber_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? blabber_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? blabber_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

// Return header video (if set)
if ( ! function_exists( 'blabber_get_header_video' ) ) {
	function blabber_get_header_video() {
		$video = '';
		if ( apply_filters( 'blabber_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( blabber_check_theme_option( 'header_video' ) ) {
				$video = blabber_get_theme_option( 'header_video' );
				if ( (int) $video > 0 ) {
...
// Show post banner
if ( ! function_exists( 'blabber_show_post_banner' ) ) {
	function blabber_show_post_banner( $banner_pos = '') {
		if ( is_singular( 'post' ) && '' !== $banner_pos ){
			$banner_code   = blabber_get_theme_option( $banner_pos . '_banner_code' );
			$banner_img    = blabber_get_theme_option( $banner_pos . '_banner_img' );
			$banner_height = blabber_get_theme_option( $banner_pos . '_banner_height' );
			$banner_class  = !empty( $banner_img )
								? ( 'background' == $banner_pos ? '' : 'banner_with_image ' )
									. blabber_add_inline_css_class( 
										'background-image:url(' . esc_url( $banner_img ) . ');'
										. ( ! empty( $banner_height )
											? 'min-height:' . blabber_prepare_css_value( $banner_height ) . ';'
											: ''
											)
										)
								: '';
			$banner_link   = blabber_get_theme_option( $banner_pos . '_banner_link' );
			if ( ! empty( $banner_code ) || ! empty( $banner_img ) ) {
				...
				blabber_show_layout( $banner_code );
...
if ( ! function_exists( 'blabber_show_post_featured_image' ) ) {
	function blabber_show_post_featured_image( $thumb_bg = false ) {
		// Featured image
		if ( ! blabber_sc_layouts_showed( 'featured' ) && strpos( blabber_get_post_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'blabber_action_before_post_featured' );
			...
			do_action( 'blabber_action_after_post_featured' );
		} elseif ( blabber_is_on( blabber_get_theme_option( 'seo_snippets' ) ) && has_post_thumbnail() ) {
			?>
...
if ( ! function_exists( 'blabber_show_post_title_and_meta' ) ) {
	function blabber_show_post_title_and_meta( $need_content_wrap = false, $need_breadcrumbs = false ) {
...
if ( ! function_exists( 'blabber_show_post_content' ) ) {
	function blabber_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$plain = true;
		...
		if ( has_excerpt() ) {
			...
			blabber_show_layout( blabber_filter_post_content( get_the_content('') ) );
			...
			$quote = blabber_get_tag( blabber_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				blabber_show_layout( wpautop( $quote ) );
				$plain = false;
			} else {
				blabber_show_layout( blabber_filter_post_content( get_the_content() ) );
			}
			...
			if ( $plain ) {
				$len = ! empty( $args['hide_excerpt'] )
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: blabber_get_theme_option( 'excerpt_length' )
								);
				$output = blabber_excerpt( $output, $len );
			}
		}
		blabber_show_layout( $output, $otag, $ctag);
	}
}


// Show post link 'Read more' in the blog posts
if ( ! function_exists( 'blabber_show_post_more_link' ) ) {
	function blabber_show_post_more_link( $args = array(), $otag='', $ctag='' ) {
		blabber_show_layout(
			'<a class="more-link sc_button sc_button_bordered" href="' . esc_url( get_permalink() ) . '">'
				. ( ! empty( $args['more_text'] )
						? esc_html( $args['more_text'] )
						: esc_html__( 'Read More', 'blabber' )
						)
			. '</a>',
			$otag,
			$ctag
...
if ( ! function_exists( 'blabber_show_post_meta' ) ) {
	function blabber_show_post_meta( $args = array() ) {
		if ( is_single() && blabber_is_off( blabber_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta<?php echo ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : ''; ?>">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ' ' ) : apply_filters( 'blabber_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						blabber_show_layout( $cats, '<span class="post_meta_item post_categories">', '</span> ');
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'blabber_filter_get_post_date', blabber_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
					if ( ! empty( $dt ) ) {
						blabber_show_layout(
							$dt,
							'<span class="post_meta_item post_date' . ( ! empty( $args['seo'] ) ? ' date updated' : '' ) . '"'
								...
								. '>'
								. ( ! is_singular() ? (! wp_doing_ajax() ? '<a href="' . esc_url( get_permalink() ) . '">' : '') : '' ),
							( ! is_singular() ? (! wp_doing_ajax() ? '</a>' : '') : '' ) . '</span> '
						);
					}
				} elseif ( 'author' == $comp ) {
					// Post author
					...
					if ( $author_id > 0 ) {
						...
						$author_avatar = get_avatar( get_the_author_meta( 'user_email', $author_id ), 32 * blabber_get_retina_multiplier() );
						echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
								. ( blabber_get_theme_setting( 'show_author_avatar' )
					...
					if ( !is_singular() || have_comments() || comments_open() ) {
						...
						echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
								...
								. '<span class="post_meta_label">' . esc_html( _n( 'Comment', 'Comments', $post_comments, 'blabber' ) ) . '</span>'
							. '</a> ';
					}
					...
					if ( function_exists( 'trx_addons_get_post_views' ) ) {
						...
						echo ( is_singular()
							...
							. ' class="post_meta_item post_meta_views trx_addons_icon-eye">'
								...
								. '<span class="post_meta_label">' . esc_html( _n( 'View', 'Views', $post_views, 'blabber' ) ) . '</span>'
							. ( is_singular()
								? '</span> '
								: '</a> '
								);
				...
				// Likes (Emotions)
					...
					if ( function_exists( 'trx_addons_get_post_likes' ) ) {
						...
						echo ( true == $emotions_allowed
								...
								: '<a href="#"'
									...
									. ' title="' . ( ! empty( $allow ) ? esc_attr__( 'Like', 'blabber') : esc_attr__( 'Dislike', 'blabber' ) ) . '"'
									. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
									. ' data-likes="' . esc_attr( $post_likes ) . '"'
									. ' data-title-like="' . esc_attr__( 'Like', 'blabber') . '"'
									. ' data-title-dislike="' . esc_attr__( 'Dislike', 'blabber' ) . '"'
									. '>'
								)
									. '<span class="post_meta_number">' . esc_html($post_likes) . '</span>'
									. '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? esc_html( _n( 'Reaction', 'Reactions', $post_likes, 'blabber' ) )
											: esc_html( _n( 'Like', 'Likes', $post_likes, 'blabber' ) )
											)
									. '</span>'
								. '</a> ';
					}
					...
					// Socials share
					blabber_show_share_links(
						array(
							'type'    => $args['share_type'],
							'caption' => 'drop' == $args['share_type'] ? esc_html__( 'Share', 'blabber' ) : '',
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'blabber' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

				} else {
					// Custom counter
					do_action( 'blabber_action_show_post_meta', $comp, get_the_ID(), $args );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'blabber_show_post_featured' ) ) {
	function blabber_show_post_featured( $args = array() ) {

		$args = array_merge(
			array(
				'hover'         => blabber_get_theme_option( 'image_hover' ), // Hover effect
				...
				'show_no_image' => blabber_is_on( blabber_get_theme_setting( 'allow_no_image' ) ),  // Display 'no-image.jpg' if post haven't thumbnail
				'seo'           => blabber_is_on( blabber_get_theme_option( 'seo_snippets' ) ),     // Add SEO-snippets
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: blabber_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? blabber_get_no_image( '', true ) : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = blabber_get_post_image();
				if ( ! empty( $image ) ) {
					$image = blabber_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . blabber_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						blabber_is_on( blabber_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( blabber_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				$output .= '>';
				blabber_show_layout( $output );
				if ( has_post_thumbnail() && $args['seo'] ) {
					$blabber_attr = blabber_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $blabber_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $blabber_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb
				...
				|| ( ! $args['thumb_only']
						&& ( in_array( $post_format, array( 'image', 'audio', 'video' ) )
							|| ( 'gallery' == $post_format && blabber_exists_trx_addons() )
							)
					)
			) {
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = blabber_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						blabber_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) || ! empty( $args['video'] ) ) ) {
					$post_content = blabber_get_post_content();
					$post_content_parsed = $post_content;

					if ( 'video' == $post_format || ! empty( $args['video'] ) ) {
						$video = ! empty( $args['video'] ) ? $args['video'] : blabber_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = blabber_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = blabber_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = blabber_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {

						    if ( $has_thumb && !empty($args['popup']) && blabber_exists_trx_addons() ) {
						        $video_go = trx_addons_get_video_layout(array(
						                                'link'  => '',
														'embed' => $video,
														'cover' => $has_thumb ? get_post_thumbnail_id() : '',
														'show_cover' => false,
														'popup' => true
														)
													);

						        if (!empty($video_go)) trx_addons_show_layout($video_go);

						    } else {

                                if ( $has_thumb ) {
                                    $video = blabber_make_video_autoplay( $video );
                                    ?>
						...
						$audio = blabber_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = blabber_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = blabber_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								if ( strpos( $audio, '<audio' ) !== false ) {
									$media_author = blabber_get_tag_attrib( $audio, '<audio>', 'data-author' );
									$media_title  = blabber_get_tag_attrib( $audio, '<audio>', 'data-caption' );
								}
								if ( empty( $media_author) &&  empty( $media_title) ) {
									$media = urldecode( blabber_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
									if ( ! empty( $media ) ) {
										...
										// Parse tag params
										$media_author = strip_tags( blabber_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
										$media_title  = strip_tags( blabber_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

									}
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php blabber_show_layout( $media_title ); ?></h5>
									<?php
								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php blabber_show_layout( $media_author ); ?></div>
									<?php
								}
								// Display audio
								blabber_show_layout( $audio );
								?>
							</div>
							<?php
						}
...
if ( ! function_exists( 'blabber_get_no_image' ) ) {
	function blabber_get_no_image( $no_image = '', $need = false ) {
		static $no_image_url = '';
		$img = blabber_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || blabber_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( empty( $no_image_url ) ) {
				$no_image_url = blabber_get_file_url( 'images/no-image.jpg' );
			}
			$img = $no_image_url;
		}
		if ( ! empty( $img ) ) {
...
if ( ! function_exists( 'blabber_add_bg_in_post_nav' ) ) {
	function blabber_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), blabber_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), blabber_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'blabber-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'blabber_show_related_posts' ) ) {
	function blabber_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : blabber_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) blabber_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = blabber_get_theme_option( 'related_position' );
			$related_style = blabber_get_theme_option( 'related_style' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						...
						esc_html_e( 'You May Also Like', 'blabber' );
					}
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = blabber_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = blabber_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = blabber_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'blabber_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					blabber_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						if ( ! apply_filters( 'blabber_filter_related_post_showed', false, $args, $style ) ) {
						get_template_part( apply_filters( 'blabber_filter_get_template_part', 'templates/related-posts', $style ), $style );
						}
						?></div><?php
					}
				?></div><?php		// .swiper-wrapper || .columns_wrap
				if ( $args['slider'] ) {
					blabber_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'blabber_show_related_posts_callback' ) ) {
	add_action( 'blabber_action_related_posts', 'blabber_show_related_posts_callback' );
	function blabber_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'blabber_filter_show_related_posts', false ) ) {
			$blabber_related_posts   = (int) blabber_get_theme_option( 'related_posts' );
			$blabber_related_columns = (int) blabber_get_theme_option( 'related_columns' );
			$blabber_related_style   = blabber_get_theme_option( 'related_style' );
			if ( (int) blabber_get_theme_option( 'show_related_posts' ) && $blabber_related_posts > 0 ) {
				blabber_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $blabber_related_posts ) ),
						'columns'        => max( 1, min( 6, $blabber_related_posts, $blabber_related_columns ) ),
					),
					$blabber_related_style
				);
			}
		}
	}
...
if ( ! function_exists( 'blabber_show_portfolio_posts' ) ) {
	function blabber_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? blabber_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = blabber_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = blabber_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$blabber_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the blabber_ajax_get_posts action
if ( ! function_exists( 'blabber_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_blabber_ajax_get_posts', 'blabber_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_blabber_ajax_get_posts', 'blabber_ajax_get_posts_callback' );
	function blabber_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'blabber' );
		}
...
// Show pagination
if ( ! function_exists( 'blabber_show_pagination' ) ) {
	function blabber_show_pagination( $args = array() ) {
		global $wp_query;
		$pagination = ! empty( $args[ 'pagination' ] )
						? $args[ 'pagination' ]
						: blabber_get_theme_option( 'blog_pagination' );
		$prefix     = ! empty( $args[ 'class_prefix' ] )
						? $args[ 'class_prefix' ]
						: 'nav';
		if ( 'pages' == $pagination ) {
			blabber_show_layout( str_replace( "\n", '', get_the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'blabber' ),
					'next_text'          => esc_html__( '>', 'blabber' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'blabber' ) . ' </span>',
				)
			)));
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="<?php echo esc_attr( $prefix ); ?>-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'blabber' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="<?php echo esc_attr( $prefix ); ?>-links-old">
				<span class="<?php echo esc_attr( $prefix ); ?>-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'blabber' ) : esc_html__( 'Newest posts', 'blabber' ) ); ?></span>
				<span class="<?php echo esc_attr( $prefix ); ?>-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'blabber' ) : esc_html__( 'Older posts', 'blabber' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'blabber_single_comments_field' ) ) {
	function blabber_single_comments_field( $args ) {
...
if ( ! function_exists( 'blabber_is_blog_style_use_masonry' ) ) {
	function blabber_is_blog_style_use_masonry( $style ) {
		$blog_styles = blabber_storage_get( 'blog_styles' );
