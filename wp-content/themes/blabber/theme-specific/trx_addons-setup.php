...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'blabber_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'blabber_trx_addons_theme_specific_components' );
	function blabber_trx_addons_theme_specific_components( $enable = false ) {
		return BLABBER_THEME_FREE
					? false     // Free version
					: false;     // Pro version or Developer mode
	}
}

if ( ! function_exists( 'blabber_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'blabber_trx_addons_theme_specific_setup1', 1 );
	function blabber_trx_addons_theme_specific_setup1() {
		if ( blabber_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'blabber_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'blabber_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'blabber_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'blabber_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'blabber_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'blabber_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'blabber_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'blabber_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'blabber_trx_addons_theme_specific_setup' );
	function blabber_trx_addons_theme_specific_setup() {
		if ( blabber_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'blabber_action_before_post_meta', 'blabber_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'blabber_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'blabber_trx_addons_cv_enable');
	function blabber_trx_addons_cv_enable( $enable = false ) {
...
// Demo mode
if ( ! function_exists( 'blabber_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'blabber_trx_addons_demo_enable');
	function blabber_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'blabber_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'blabber_trx_addons_edd_themes_market_enable')) {
	function blabber_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'blabber_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'blabber_trx_addons_api_list');
	function blabber_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'blabber_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'blabber_trx_addons_cpt_list');
	function blabber_trx_addons_cpt_list( $list = array() ) {
        ...
        foreach ( $list as $k => $v ) {
            if ( $k == 'team' ) {
                $list[ $k ]['layouts_arh']['featured_3'] = esc_html__('Featured /3 columns/', 'blabber');
...
// Shortcodes
if ( ! function_exists( 'blabber_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'blabber_trx_addons_sc_list');
	function blabber_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'blabber_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'blabber_trx_addons_widgets_list');
	function blabber_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'blabber_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'blabber_trx_addons_menu_cache' );
	function blabber_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'blabber_trx_addons_localize_script' ) ) {
	...
	function blabber_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = blabber_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'blabber_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'blabber_trx_addons_sc_type', 10, 2 );
	function blabber_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = blabber_array_merge( $list, blabber_get_list_blog_styles( false, 'sc' ) );
		}
...
if ( ! function_exists( 'blabber_trx_addons_sc_prepare_atts' ) ) {
    add_filter( 'trx_addons_filter_sc_prepare_atts', 'blabber_trx_addons_sc_prepare_atts', 10, 2 );
    function blabber_trx_addons_sc_prepare_atts( $atts, $sc ) {
        if ( 'trx_sc_blogger' == $sc ) {
            $list = blabber_get_list_blog_styles( false, 'sc' );
            if ( isset( $list[ $atts['type'] ] ) ) {
                ...
                if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
                    $blog_id = blabber_get_custom_blog_id( $atts['type'] );
                    ...
                    $use_masonry = blabber_is_blog_style_use_masonry( $atts['type'] );
                }
...
if ( ! function_exists( 'blabber_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'blabber_trx_addons_sc_atts', 10, 2 );
	function blabber_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'blabber_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'blabber_trx_addons_sc_map', 10, 2 );
	function blabber_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'blabber' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'blabber' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( blabber_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'blabber' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'blabber' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( blabber_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'blabber_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'blabber_trx_addons_sc_output', 10, 4 );
	function blabber_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! blabber_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'blabber_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'blabber_trx_addons_sc_item_link_classes', 10, 3 );
	function blabber_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! blabber_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'blabber_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'blabber_trx_addons_sc_item_title_tag' );
	function blabber_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'blabber_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'blabber_trx_addons_sc_item_button_args', 10, 3 );
	function blabber_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'blabber_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'blabber_trx_addons_sc_googlemap_styles' );
	function blabber_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'blabber' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'blabber_trx_addons_action_before_post_meta' ) ) {
	//Handler of the add_action( 'blabber_action_before_post_meta', 'blabber_trx_addons_action_before_post_meta');
	function blabber_trx_addons_action_before_post_meta() {
		if ( blabber_exists_trx_addons() ) {
...
if ( ! function_exists( 'blabber_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'blabber_trx_addons_tiny_mce_style_formats' );
	function blabber_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'blabber' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'blabber' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'blabber_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'blabber_filter_allow_override_header_image', 'blabber_trx_addons_allow_override_header_image' );
	function blabber_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					blabber_is_team_page()
					|| blabber_is_cars_page()
					|| blabber_is_cars_agents_page()
					|| blabber_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'blabber_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'blabber_trx_addons_thumb_size', 10, 2 );
	function blabber_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
		//      Uncomment next lines to change team members images (default is 'avatar')
		//      if ($type == 'team-default')
		//          $thumb_size = blabber_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'blabber_trx_addons_meta_box_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'blabber_trx_addons_meta_box_fields', 10, 2 );
	function blabber_trx_addons_meta_box_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'blabber' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'blabber' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
...
if ( ! function_exists( 'blabber_filter_get_list_menu_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_menu_hover', 'blabber_filter_get_list_menu_hover' );
    function blabber_filter_get_list_menu_hover( $list ) {
        unset( $list['fade_box'] );
...
if ( ! function_exists( 'blabber_filter_get_list_input_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_input_hover', 'blabber_filter_get_list_input_hover' );
    function blabber_filter_get_list_input_hover( $list ) {
        unset( $list['accent'] );
        unset( $list['path'] );
        unset( $list['jump'] );
...
if ( ! function_exists( 'blabber_filter_icons_change_param' ) ) {
    add_filter( 'trx_addons_sc_param_group_params', 'blabber_filter_icons_change_param', 10, 2 );
    function blabber_filter_icons_change_param( $list, $sc ) {
        if($sc == 'trx_sc_icons'){
            if (is_array($list)) {
                foreach($list as $k=>$v) {
                    if ($v['name'] == 'image') {
                        $list[$k]['label'] = esc_html__('image', 'blabber');
                        unset($list[$k]['condition']['icon']);
                        break;
                    }
                }
            }
...
if ( ! function_exists( 'blabber_filter_team_posts_title' ) ) {
    add_filter( 'trx_addons_filter_team_posts_title', 'blabber_filter_team_posts_title' );
    function blabber_filter_team_posts_title( ) {
     return esc_html__('Contact me', 'blabber');
    }
