<?php
/**
 * Skins support: Gaming skin file for the skin 'Default'
 *
...
if ( ! function_exists( 'blabber_skin_importer_set_options' ) ) {
	...
	function blabber_skin_importer_set_options($options = array()) {
		if ( is_array( $options ) ) {
			$options['demo_type'] = 'gaming';
			$options['files']['gaming'] = $options['files']['default'];
			$options['files']['gaming']['title'] = esc_html__('Gaming Demo', 'blabber');
			$options['files']['gaming']['domain_dev'] = '';    // Developers domain
			$options['files']['gaming']['domain_demo'] = esc_url( blabber_get_protocol() . '://gaming.blabber.ancorathemes.com' );   // Demo-site domain
