...
?><article 
	...
	if ( !in_array($post_format, array('link', 'aside', 'status', 'quote')) ) {
		...
		<div class="post_header entry-header">
			<?php
            if ( $number<=$featured && has_post_thumbnail() ) {
                ?><div class="post_meta_top"><span class="post_categories"> <?php echo trx_addons_get_post_categories() ?> </span></div><?php
            }
			...
			if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) && $number <= $featured && has_post_thumbnail() ) {
				?><div class="post_meta"><span class="post_date"><a href="<?php echo esc_url(get_permalink()); ?>"><?php echo get_the_date(); ?></a></span></div><?php
			}
			?>
		</div><!-- .entry-header -->
		<?php
