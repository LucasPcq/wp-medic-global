...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $blabber_post_format ) ); ?>>
	...
	blabber_show_post_featured(
		array(
			'thumb_size'    => apply_filters( 'blabber_filter_related_thumb_size', blabber_get_thumb_size( (int) blabber_get_theme_option( 'related_posts' ) == 1 ? 'huge' : 'med' ) ),
            'class' => (in_array( get_post_type(), array( 'post', 'attachment' ) ) ? 'width_cat_top' : ''),
            'post_info' => blabber_show_post_meta( array( 'components' => 'rating', 'seo' => false, 'class' => 'top_rating'))
            )
	);
