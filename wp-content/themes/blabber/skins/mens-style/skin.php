...
if ( ! function_exists( 'blabber_skin_importer_set_options' ) ) {
	...
	function blabber_skin_importer_set_options($options = array()) {
		if ( is_array( $options ) ) {
			$options['demo_type'] = 'mens-style';
			$options['files']['mens-style'] = $options['files']['default'];
			$options['files']['mens-style']['title'] = esc_html__('Mens & Style Demo', 'blabber');
			$options['files']['mens-style']['domain_dev'] = '';    // Developers domain
			$options['files']['mens-style']['domain_demo'] = esc_url( blabber_get_protocol() . '://mens-style.blabber.ancorathemes.com' );   // Demo-site domain
