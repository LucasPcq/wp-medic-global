...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $blabber_post_format ) ); ?>>
    ...
    $thumb_size = blabber_get_thumb_size( (int) blabber_get_theme_option( 'related_posts' ) == 1 ? 'huge'  : ($blabber_post_format === 'video' ? 'medium' : 'box' ) );
    if($blabber_post_format === 'video' || $blabber_post_format === 'gallery'){
        $thumb_size = blabber_get_thumb_size('medium' );
    }
