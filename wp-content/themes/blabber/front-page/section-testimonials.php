<div class="front_page_section front_page_section_testimonials<?php
	$blabber_scheme = blabber_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $blabber_scheme ) && ! blabber_is_inherit( $blabber_scheme ) ) {
		echo ' scheme_' . esc_attr( $blabber_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( blabber_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( blabber_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$blabber_css      = '';
		$blabber_bg_image = blabber_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$blabber_anchor_icon = blabber_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$blabber_anchor_text = blabber_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( blabber_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$blabber_css      = '';
			$blabber_bg_mask  = blabber_get_theme_option( 'front_page_testimonials_bg_mask' );
			$blabber_bg_color_type = blabber_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $blabber_bg_color_type ) {
				$blabber_bg_color = blabber_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$blabber_caption = blabber_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $blabber_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $blabber_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $blabber_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$blabber_description = blabber_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $blabber_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $blabber_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $blabber_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
				...
				if ( ! blabber_exists_trx_addons() ) {
					...
					blabber_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
				}
			}
			?>
			</div>
