...
?>><?php
	...
	foreach ($args['icons'] as $item) {
        ...
        <?php }
			...
			if (!empty($item['image'])) {
				$item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', blabber_get_thumb_size('masonry-big'), 'icons-default'));
				if (!empty($item['image'])) {
                    $class=  'post_featured_bg' . ' ' . blabber_add_inline_css_class( 'background-image: url(' . esc_url( $item['image'] ) . ');' );
					?><div class="sc_icons_image <?php echo esc_attr($class); ?>"></div><?php
				}
			}
