...
$row_content_width = $anchor = $content_text_aligment = $simple_background_color = $simple_background_image = $background_image_position = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = $row_background_lines = '';
...
if ( $row_content_width === 'grid' ) {
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . overton_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
	$mkdf_row_wrapper_end   .= '</div></div>';
}

$enable_lines = $row_background_lines;

$html = '';
if($enable_lines === 'yes'){
    $html .= '<div class="mkdf-grid-lines-holder mkdf-grid-columns-4">';
    for ($i = 1; $i <= 4; $i++) {
        $html .= '<div class="mkdf-grid-line mkdf-grid-column-' . $i . '"></div>';
    }
    $html .= '</div>';
}
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . overton_mikado_get_inline_style( $mkdf_vc_row_styles ) . '>';
$output .= $html;
