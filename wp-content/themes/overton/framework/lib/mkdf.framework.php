...
/*
   Class: OvertonMikadoClassFramework
...
class OvertonMikadoClassFramework {
	...
	private function __construct() {
		$this->mkdOptions         = OvertonMikadoClassOptions::get_instance();
		$this->mkdMetaBoxes       = OvertonMikadoClassMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = OvertonMikadoClassTaxonomyOptions::get_instance();
		$this->mkdUserOptions     = OvertonMikadoClassUserOptions::get_instance();
		$this->mkdDashboardOptions = OvertonMikadoClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( OvertonMikadoClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class OvertonMikadoClassSkinManager
 *
...
class OvertonMikadoClassSkinManager {
	...
	/**
	 * @see OvertonMikadoClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = MIKADO_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'overton' );
...
/**
 * Class OvertonMikadoClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class OvertonMikadoClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( overton_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo overton_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'overton' ) ?>';
		</script>
	<?php }
...
/*
   Class: OvertonMikadoClassOptions
...
class OvertonMikadoClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $overton_mikado_global_options ) ) {
			return $overton_mikado_global_options[ $key ];
...
/*
   Class: OvertonMikadoClassAdminPage
   A class that initializes Mikado Admin Page
*/
class OvertonMikadoClassAdminPage implements iOvertonMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new OvertonMikadoClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iOvertonMikadoInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: OvertonMikadoClassMetaBoxes
...
/*
   Class: OvertonMikadoClassMetaBox
   A class that initializes Mikado Meta Box
*/
class OvertonMikadoClassMetaBox implements iOvertonMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $overton_mikado_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: OvertonMikadoClassTaxonomyOptions
   A class that initializes OvertonMikadoClass Taxonomy Options
*/
class OvertonMikadoClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: OvertonMikadoClassTaxonomyOption
   A class that initializes OvertonMikadoClass Taxonomy Option
*/
class OvertonMikadoClassTaxonomyOption implements iOvertonMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new OvertonMikadoClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: OvertonMikadoClassUserOptions
   A class that initializes OvertonMikadoClass User Options
*/
class OvertonMikadoClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: OvertonMikadoClassUserOption
   A class that initializes OvertonMikadoClass User Option
*/
class OvertonMikadoClassUserOption implements iOvertonMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new OvertonMikadoClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: OvertonMikadoClassDashboardOptions
   A class that initializes OvertonMikadoClass Dashboard Options
*/
class OvertonMikadoClassDashboardOptions {
	private static $instance;
...
/*
   Class: OvertonMikadoClassDashboardOption
   A class that initializes OvertonMikadoClass Dashboard Option
*/
class OvertonMikadoClassDashboardOption implements iOvertonMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new OvertonMikadoClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'overton_mikado_init_framework_variable' ) ) {
	function overton_mikado_init_framework_variable() {
		global $overton_mikado_global_Framework;
		
		$overton_mikado_global_Framework = OvertonMikadoClassFramework::get_instance();
		$mkdSkinManager                 = new OvertonMikadoClassSkinManager();
		$overton_mikado_global_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'overton_mikado_action_before_options_map', 'overton_mikado_init_framework_variable' );
}
?>