...
/*
   Class: OvertonMikadoClassDashboardForm
   A class that initializes Mikado Dashboard Container
*/
class OvertonMikadoClassDashboardForm implements iOvertonMikadoInterfaceLayoutNode, iOvertonMikadoInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render($factory) {
		...
		<form method="<?php echo esc_attr($this->form_method);?>" id="<?php echo esc_attr($this->form_id);?>" class="<?php echo esc_attr($action_class)?>" data-action="<?php echo esc_attr($this->form_action);?>">
			...
			if ( overton_mikado_core_plugin_installed() ) {
				echo overton_mikado_get_button_html( array(
...
/*
   Class: OvertonMikadoClassDashboardGroup
   A class that initializes OvertonMikadoClass Group Field
*/
class OvertonMikadoClassDashboardGroup implements iOvertonMikadoInterfaceLayoutNode, iOvertonMikadoInterfaceRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: OvertonMikadoClassDashboardTitle
   A class that initializes Dashboard Title
*/
class OvertonMikadoClassDashboardTitle implements iOvertonMikadoInterfaceRender {
	private $name;
	private $title;
...
/*
   Class: OvertonMikadoClassDashboardField
   A class that initializes OvertonMikadoClass Front Field
*/

class OvertonMikadoClassDashboardField implements iOvertonMikadoInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class OvertonMikadoClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class OvertonMikadoClassDashboardFieldText extends OvertonMikadoClassDashboardFieldType {
...
class OvertonMikadoClassDashboardFieldTextArea extends OvertonMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class OvertonMikadoClassDashboardFieldImage extends OvertonMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
		...
		<div class="mkdf-dashboard-field-holder <?php echo esc_attr($class); ?>">
			...
			<div class="mkdf-dashboard-gallery-holder">
				...
				<div class="mkdf-dashboard-gallery-uploader">
					<?php 
					if ( overton_membership_theme_installed() ) {
						echo overton_mikado_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'overton' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'overton' ) . '</span></a>';
					} ?>
					<input class="mkdf-dashboard-gallery-upload-hidden" type="file" name="<?php echo esc_html( $name ); ?>" id="<?php echo esc_html( $name ); ?>"
					       value="">
					<input type="hidden" class="mkdf-dashboard-media-hidden" name="<?php echo esc_html( $hidden_name ); ?>" value="<?php echo esc_attr($value);?>"/>
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e('Remove Media','overton'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class OvertonMikadoClassDashboardFieldGallery extends OvertonMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class OvertonMikadoClassDashboardFieldSelect extends OvertonMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class OvertonMikadoClassDashboardFieldDate extends OvertonMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class OvertonMikadoClassDashboardFieldIcon extends OvertonMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = overton_mikado_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = overton_mikado_icon_collections()->getIconCollectionsKeys();
				...
				<div class="mkdf-dashboard-item">
					...
					<div class="mkdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = overton_mikado_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
	                        $style       = 'display: none';
							...
							<div class="mkdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>">
									<?php
									$icons = overton_mikado_icon_collections()->getIconCollection( $icons_collection );
...
class OvertonMikadoClassDashboardFieldColor extends OvertonMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class OvertonMikadoClassDashboardFieldCheckBoxGroup extends OvertonMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class OvertonMikadoClassDashboardFieldAddress extends OvertonMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="mkdf-dashboard-field-row">
				<div class="mkdf-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="mkdf-dashboard-input-holder">
						...
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'overton' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class OvertonMikadoClassDashboardRepeater implements iOvertonMikadoInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'overton' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="mkdf-dashboard-repeater-table-heading">
					<div class="mkdf-dashboard-repeater-fields-holder">
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'overton' ) ?></div>
						...
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'overton' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class OvertonMikadoClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new OvertonMikadoClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new OvertonMikadoClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new OvertonMikadoClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new OvertonMikadoClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new OvertonMikadoClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new OvertonMikadoClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new OvertonMikadoClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new OvertonMikadoClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new OvertonMikadoClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new OvertonMikadoClassDashboardFieldAddress();
