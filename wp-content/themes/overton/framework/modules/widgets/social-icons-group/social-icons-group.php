...
class OvertonMikadoClassClassIconsGroupWidget extends OvertonMikadoClassWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_social_icons_group_widget',
			esc_html__( 'Overton Social Icons Group Widget', 'overton' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'overton' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			overton_mikado_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'overton' ),
					'options' => array(
						''             => esc_html__( 'Default', 'overton' ),
						'square-icons' => esc_html__( 'Square', 'overton' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'overton' ),
					'description' => esc_html__( 'This applies to the square layout', 'overton' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'overton' ),
						'light-skin' => esc_html__( 'Light Skin', 'overton' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'overton' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'overton' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo overton_mikado_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php overton_mikado_inline_style( $icon_styles ) ?>
