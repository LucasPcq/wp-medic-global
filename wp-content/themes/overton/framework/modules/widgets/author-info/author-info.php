...
class OvertonMikadoClassAuthorInfoWidget extends OvertonMikadoClassWidget {
	public function __construct() {
		parent::__construct(
			'mkdf_author_info_widget',
			esc_html__( 'Overton Author Info Widget', 'overton' ),
			array( 'description' => esc_html__( 'Add author info element to widget areas', 'overton' ) )
		);
	...
	protected function setParams() {
		$this->params = array(
			...
			array(
				...
				'title' => esc_html__( 'Author Username', 'overton' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		<div class="widget mkdf-author-info-widget <?php echo esc_html( $extra_class ); ?>" <?php echo overton_mikado_get_inline_style( $widget_styles ); ?>>
			<div class="mkdf-aiw-inner">
				<a itemprop="url" class="mkdf-aiw-image" href="<?php echo esc_url( get_author_posts_url( $authorID ) ); ?>">
					<?php echo overton_mikado_kses_img( get_avatar( $authorID, 138 ) ); ?>
				</a>
				<?php if ( $author_info !== "" ) { ?>
					<h4 class="mkdf-aiw-title"><?php esc_html_e( 'About Author', 'overton' ); ?></h4>
