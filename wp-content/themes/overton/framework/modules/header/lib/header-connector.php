...
namespace OvertonMikadoNamespace\Modules\Header\Lib;

/**
 * Class OvertonMikadoClassHeaderConnector
 *
...
class OvertonMikadoClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'overton_mikado_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'overton_mikado_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'overton_mikado_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'overton_mikado_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to overton_mikado_filter_title_content_padding filter
	 *
	 * @return int
	 */
