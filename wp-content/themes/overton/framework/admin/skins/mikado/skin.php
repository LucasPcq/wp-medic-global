...
class MikadoSkin extends OvertonMikadoClassSkinAbstract {
	
	/**
	 * Skin constructor. Hooks to overton_mikado_admin_scripts_init and overton_mikado_enqueue_admin_styles
	 */
	public function __construct() {
		$this->skinName = 'mikado';
		
		//hook to
		add_action( 'overton_mikado_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'overton_mikado_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		add_action( 'overton_mikado_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'overton_mikado_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		add_action( 'overton_mikado_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'overton_mikado_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		add_filter( 'overton_mikado_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			overton_mikado_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            overton_mikado_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    		= overton_mikado_get_admin_tab();
        $active_page 	= $overton_mikado_global_Framework->mkdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see OvertonMikadoClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param OvertonMikadoClassAdminPage $page current page to load
	...
	public function renderBackupOptions() { ?>
		<div class="mkdf-options-page mkdf-page">
			<?php $this->getHeader(overton_mikado_get_theme_info_item('Name'), overton_mikado_get_theme_info_item('Version'), false); ?>
			<div class="mkdf-page-content-wrapper">
				<div class="mkdf-page-content">
					<div class="mkdf-page-navigation mkdf-tabs-wrapper vertical left clearfix">
